    ...
    app.post(['/api/user/login'], async (req, res, next) => {
        ...
        try {
            ...
            if (isMatch) {
                ...
                hostCfg.authenticated = true;
                res.status(200).json({ token: token, username: username, fullname: fullname, user_type: user_type, organization_code: organization_code, userId: idd, organization_name: organization_name });
            } else {
                // return res.status(401).json({ message: "نام کاربری یا کلمه عبور اشتباه است" });
                const error = new Error("نام کاربری یا کلمه عبور اشتباه است");
    ...
    app.get(['/api/user/partners'], authenticated2, async (req, res, next) => {
        try {
            if (req.userType == "supervisor" || req.userType == "admin") {
                ...
                const {code} = req.body;
                if(!code){
                    const error = new Error("کد سازمانی الزامی میباشد");
                    error.statusCode = 400;
                    throw error;
                }
                // const partners = await User.find({ organization_code: userLogged.organization_code });
                const partners = await User.find({ organization_code: code });
                var p = [];
                ...
                if (partners.length > 0) {
                    ...
                    const error = new Error("این سازمان هنوز کاربر ثبت شده‌ای ندارد");
                    error.statusCode = 404;
                    throw error;
                }
            } else {
